...
file = open("thousandpassword.txt", "a")
...
def cons(password):
    ...
    #2
    elif security_s(s) == True and secyurity_n(n) == True and security_l(l) == True and security_u(u) == False :
...
x = True
while x == True:
    yes_no = input("\033[1;37m Do you accept the terms and want to continue(Yes/No) \n")
...
while True:
    password = getpass.getpass(prompt = "\033[1;37mEnter your password \n")
    if len(password) >= 5:
        slow_type("Password is being compared to the million most used passwords and to english words...")
        break
    else:
...
if f == 1 and z == 1:
    ...
    slow_type("\nYour password is one of the million most used passwords: ")
    time.sleep(3)
    print("\n-This means that it would take a hacker less than a minute to get your password by running a dictionary attack")
    time.sleep(3)
